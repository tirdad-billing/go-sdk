...
func newCoupons(rootSDK *Tirdad, sdkConfig config.SDKConfiguration, hooks *hooks.Hooks) *Coupons {
	return &Coupons{
		rootSDK:          rootSDK,
...
func (s *Coupons) GetCouponByCode(ctx context.Context, code string, opts ...dtos.Option) (*dtos.GetCouponByCodeResponse, error) {
	...
	opURL, err := utils.GenerateURL(ctx, baseURL, "/coupons/code/{code}", request, nil)
	if err != nil {
		return nil, fmt.Errorf("error generating URL: %w", err)
	}
	...
	res := &dtos.GetCouponByCodeResponse{
		HTTPMeta: types.HTTPMetadata{
			Request:  req,
			Response: httpRes,
		},
