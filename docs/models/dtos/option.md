...
```go
tirdad.WithServerURL("https://api.example.com")
```
...
```go
tirdad.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
tirdad.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
tirdad.WithTimeout(30 * time.Second)
```
