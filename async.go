...
Depends on SDK types: *Tirdad, types.IngestEventRequest, Events.IngestEvent.
*/
...
// AsyncClient provides an enhanced asynchronous client for the Tirdad API
...
func (c *Tirdad) NewAsyncClient() *AsyncClient {
...
func (c *Tirdad) NewAsyncClientWithConfig(config AsyncConfig) *AsyncClient {
	...
	ac := &AsyncClient{
		...
		debugf: func(format string, args ...interface{}) {
			if config.Debug {
				fmt.Printf("[Tirdad Debug] "+format+"\n", args...)
			}
		},
		errorCallback: func(err error) {
			if config.Debug {
				fmt.Printf("[Tirdad Error] %v\n", err)
			}
		},
	}
